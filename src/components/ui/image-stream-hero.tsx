...
export type StreamImage = {
  ...
  alt?: string;
  /** Optional class override for this specific card container. */
  cardClassName?: string;
  /** Optional class override for this specific image element. */
  imgClassName?: string;
};
...
export type ImageStreamHeroProps = {
  ...
  className?: string;
  /** Additional classes applied to individual cards (unless overridden by the image). */
  cardClassName?: string;
  /** Additional classes applied to the image element inside the cards (unless overridden by the image). */
  imgClassName?: string;
};
          ...
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              return (
                <div
                  key={`${name}-${i}`}
                  className={cn(card, "absolute overflow-hidden", img?.cardClassName ?? cardClassName)}
                  style={{
                    left: "50%",
                    top: `${axis}%`,
                ...
                >
                  {img ? (
                    <img
                      ...
                      loading="lazy"
                      decoding="async"
                      className={cn("h-full w-full object-cover", img?.imgClassName ?? imgClassName)}
                      draggable={false}
                    />
                  ) : null}
                </div>
