...
import { GlobeInteractive } from "@/components/ui/GlobalInteractive";
import React from "react";

// ─── 5 orbit rings with real partner/tech logos ───────────────────────────────
// Radii increments were tightened up (was ~80-90px apart on desktop, now
// ~110px) so the rings actually read as separate rings instead of a single
// crowded band. Icon/pad sizes bumped slightly to match.
const orbits = [
  {
    radiusPx: { mobile: 62, desktop: 210 },
    duration: 16,
    icons: [
  ...
  {
    radiusPx: { mobile: 90, desktop: 320 },
    duration: 22,
    icons: [
  ...
  {
    radiusPx: { mobile: 118, desktop: 430 },
    duration: 30,
    icons: [
  ...
  {
    radiusPx: { mobile: 146, desktop: 540 },
    duration: 38,
    icons: [
  ...
  {
    radiusPx: { mobile: 174, desktop: 650 },
    duration: 48,
    icons: [
...
const OrbitRing = ({
  ...
  // Mobile only: wordmark logos (Samsung, Cisco, Autodesk…) are wide, so a
  // 22px square box shrinks them to a few px tall. Give them a landscape box.
  const imgW = mobile ? Math.round(iconSize * 1.35) : iconSize;
  const imgH = iconSize;
  ...
  return (
    <div
      style={{
        ...
        borderRadius: "50%",
        border: `${borderWidth} solid rgba(77, 77, 77, 0.18)`,
        pointerEvents: "none",
      }}
    >
      {icons.map((icon, i) => (
...
// ─── Optional caption rendered just above the globe ───────────────────────────
const CenterLabel = ({
  eyebrow,
  title,
  globeSize,
  ...
  <div
    style={{
      ...
      top: "50%",
      left: "50%",
      transform: `translate(-50%, calc(-50% - ${globeSize / 2 + (mobile ? 20 : 34)}px))`,
      zIndex: 11,
      textAlign: "center",
      pointerEvents: "none",
      whiteSpace: "nowrap",
...
export default function OrbitingCirclesGlobeDemo({
  ...
  mobile?: boolean;
  // Optional caption ("OUR TRUSTED / TECHNOLOGY ECOSYSTEM") rendered just
  // above the globe, inside the orbit stack, so it moves as one unit with it.
  centerLabel?: { eyebrow: string; title: string };
}) {
  // Bumped up from 320/120 so the globe holds its own against the wider
  // orbit rings instead of shrinking into a small dot at the center.
  const globeSize = mobile ? 130 : 360;
  const iconSize  = mobile ? 22  : 48;
  const padSize   = mobile ? 42  : 90;
    ...
    >
      ...
      <div
        style={{
          ...
          height: globeSize,
          zIndex: 10,
          // "auto" (not "none") — the cobe globe is drag-to-rotate, so it
          // needs to actually receive pointer events, unlike the old
          // decorative particle sphere.
          pointerEvents: "auto",
        }}
      >
        <GlobeInteractive className="h-full w-full" />
      </div>
